...
class Model(torch.nn.Module):
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        if not args.get("resume"):
            # 直接使用当前加载的模型，避免按 yaml 重建回未剪枝结构
            self.trainer.model = self.model
            self.model = self.trainer.model

        print("-----------------------------------")
        print(f"\033[1;32mINFO\033[0m: custom_model is True, load pruned/custom model directly.")
    ...
    def _reset_ckpt_args(args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Examples:
            ...
            >>> print(reset_args)
            {'imgsz': 640, 'data': 'coco.yaml', 'task': 'detect','model_t'}
        """
