...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        # 兼容旧写法 Distillation=... 和新写法 model_t=...
        self.model_t = overrides.get("model_t", overrides.get("Distillation", None))
        self.distill_feat_type = overrides.get("distill_feat_type", overrides.get("loss_type", "cwd"))
        self.distillonline = overrides.get("distillonline", False)
        self.logit_loss = overrides.get("logit_loss", False)

        layers = overrides.get("distill_layers", overrides.get("layers", [6, 8, 15, 20, 23, 26]))
        if isinstance(layers, (list, tuple)):
            self.distill_layers = [int(x) for x in layers]
        else:
